...
function extractBusinessInfo(text: string, url: string): ExtractedContent["business"] {
  const business: ExtractedContent["business"] = {}

  // Extract business name from domain or title
  try {
    const domain = new URL(url).hostname.replace("www.", "")
    business.name = domain.split(".")[0].charAt(0).toUpperCase() + domain.split(".")[0].slice(1)
  } catch {
  ...
  if (services.length === 0) {
    services.push(
      ...
      },
      {
        ...
        description: "Expert advice and consultation to help you achieve your goals.",
      },
      {
        title: "Custom Solutions",
        description: "Customized solutions designed specifically for your requirements.",
      },
    )
  }
...
function extractNavigation($: cheerio.CheerioAPI): ExtractedContent["navigation"] {
  const navigation: ExtractedContent["navigation"] = []

  // Look for navigation elements
  const navSelectors = ["nav a", ".nav a", ".navigation a", ".menu a", "header a"]
  ...
  // Remove duplicates
  const uniqueNav = navigation.filter((item, index, self) => index === self.findIndex((t) => t.text === item.text))
...
export async function POST(request: NextRequest) {
  try {
    ...
    const extractedContent: ExtractedContent = {
      title: safeString($("title").text() || $("h1").first().text() || "Website"),
    ...
    if (!extractedContent.description) {
      ...
      if (firstParagraph && firstParagraph.length > 50) {
        ...
        extractedContent.description = `Professional ${extractedContent.business?.industry || "business"} services`
      }
    }

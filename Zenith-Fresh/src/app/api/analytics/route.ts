...
function calculateAnalyticsStats(analyses: any[]) {
  ...
  const performanceTrends = {
    ...
    coreWebVitals: {
      lcp: calculateAverage(analyses.map(a => a.coreWebVitals?.largestContentfulPaint).filter(Boolean)),
      fid: calculateAverage(analyses.map(a => a.coreWebVitals?.firstInputDelay).filter(Boolean)),
      cls: calculateAverage(analyses.map(a => a.coreWebVitals?.cumulativeLayoutShift).filter(Boolean)),
    },
    scores: {
      performance: calculateAverage(analyses.map(a => a.performanceMetrics ? 
        (a.performanceMetrics.cacheScore + a.performanceMetrics.compressionScore) / 2 : null).filter(Boolean)),
      accessibility: calculateAverage(analyses.map(a => a.technicalChecks?.accessibilityScore).filter(Boolean)),
      seo: calculateAverage(analyses.map(a => a.technicalChecks?.seoScore).filter(Boolean)),
      security: calculateAverage(analyses.map(a => a.technicalChecks?.securityScore).filter(Boolean)),
    },
  };

...
export class PerformanceAudit {
  ...
  async audit(websiteUrl: string) {
    ...
    try {
      ...
      (results as any).error = error instanceof Error ? error.message : 'Unknown error';
      return results;
    }
  }

  async measurePageLoadTime(url: string, results: any) {
    try {
      ...
      results.checks.pageLoadTime = { score: 0, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }

  async analyzeResourceSizes(url: string, results: any) {
    try {
      const response = await fetch(url);
      const html = await response.text();
      ...
      results.checks.resourceSizes = { score: 0, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }
  ...
  async checkCoreWebVitals(url: string, results: any) {
    try {
      ...
      results.checks.coreWebVitals = { score: 0, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }

  async analyzeImageOptimization(url: string, results: any) {
    try {
      const response = await fetch(url);
      const html = await response.text();
      ...
      results.checks.imageOptimization = { score: 0, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }

  async checkCaching(url: string, results: any) {
    try {
      const response = await fetch(url);
      ...
      results.checks.caching = { score: 0, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }

  async analyzeJavaScript(url: string, results: any) {
    try {
      const response = await fetch(url);
      const html = await response.text();
      ...
      results.checks.javascript = { score: 0, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }

  async checkCSS(url: string, results: any) {
    try {
      const response = await fetch(url);
      const html = await response.text();
      ...
      results.checks.css = { score: 0, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }

  async analyzeFonts(url: string, results: any) {
    try {
      const response = await fetch(url);
      const html = await response.text();
      ...
      results.checks.fonts = { score: 0, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }

  async checkCompression(url: string, results: any) {
    try {
      const response = await fetch(url);
      ...
      results.checks.compression = { score: 0, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  }

  // Utility methods
  extractResources(html: string, regex: RegExp): string[] {
  ...
  extractTTFB(response: Response): number {
    // In real implementation, measure actual TTFB
    return Math.random() * 500 + 100; // Simulate 100-600ms
  }

  simulateLCP(): number { return Math.random() * 3000 + 1000; } // 1-4s
  simulateFID(): number { return Math.random() * 200 + 50; } // 50-250ms
  simulateCLS(): number { return Math.random() * 0.3; } // 0-0.3
  simulateFCP(): number { return Math.random() * 2000 + 800; } // 0.8-2.8s
  simulateTTI(): number { return Math.random() * 4000 + 2000; } // 2-6s

  // Scoring methods
  calculateLoadTimeScore(checks: any): number {
  ...
  calculateResourceSizeScore(checks: any): number {
    let score = 100;
  ...
  calculateCoreWebVitalsScore(checks: any): number {
    let score = 0;
  ...
  calculateImageOptimizationScore(checks: any): number {
    let score = 0;
  ...
  calculateCachingScore(checks: any): number {
    let score = 0;
  ...
  calculateJavaScriptScore(checks: any): number {
    let score = 100;
  ...
  calculateCSSScore(checks: any): number {
    let score = 100;
  ...
  calculateFontsScore(checks: any): number {
    let score = 100;
  ...
  calculateCompressionScore(checks: any): number {
    if (checks.hasBrotli) return 100;
    if (checks.hasGzip) return 80;
    return 0;
  }

  calculateScore(checks: any): number {
    const weights = {
      pageLoadTime: 0.2,
      coreWebVitals: 0.25,
      resourceSizes: 0.15,
